...
CREATE TABLE TABLE_BOARD_freeTalk(
	...
	BOARD_DATE DATE,
	CONSTRAINT BOARD_freeTalk_PK PRIMARY KEY(BOARDNUM),
	CONSTRAINT BOARD_freeTalk_FK FOREIGN KEY(BOARDID) REFERENCES TABLE_USER(MEMBERID)
);
...
CREATE TABLE TABLE_FILES_freeTalk (	
	...
	BOARDNUM NUMBER(10),
	CONSTRAINT FILES_freeTalk_PK PRIMARY KEY(FILENAME),
	CONSTRAINT FILES_freeTalk_FK FOREIGN KEY(BOARDNUM) REFERENCES TABLE_BOARD_freeTalk(BOARDNUM)
);
...
CREATE TABLE TABLE_REPLY_freeTalk(
   ...
   CONSTRAINT REPLY_freeTalk_PK PRIMARY KEY(REPLYNUM),
   CONSTRAINT REPLY_freeTalk_FK FOREIGN KEY(BOARDNUM) REFERENCES TABLE_BOARD_freeTalk(BOARDNUM),
   CONSTRAINT REPLY_freeTalk_MEMBER_FK FOREIGN KEY(MEMBERID) REFERENCES TABLE_USER(MEMBERID)
);
...
insert into TABLE_BOARD_POINT values (BOARD_POINT_SEQ.NEXTVAL, current_timestamp, 5000, '5000포인트 결제' , 'hds1234');
...
CREATE TABLE TABLE_BOARD_EVENT(
	...
	CONSTRAINT BOARD_EVENT_PK PRIMARY KEY(BOARDNUM),
	CONSTRAINT BOARD_EVENT_FK FOREIGN KEY(BOARDID) REFERENCES TABLE_USER(MEMBERID)
);
...
CREATE TABLE TABLE_FILES_EVENT(
	FILENAME varchar2(2000),
	BOARDNUM NUMBER(10),
	CONSTRAINT FILES_EVENT_PK PRIMARY KEY(FILENAME),
	CONSTRAINT FILES_EVENT_FK FOREIGN KEY(BOARDNUM) REFERENCES TABLE_BOARD_EVENT(BOARDNUM)
);
...
CREATE TABLE TABLE_REPLY_EVENT (
   ...
   MEMBERID varchar2(100),
   REPLYCONTENT varchar2(4000),
   CONSTRAINT REPLY_EVENT_PK PRIMARY KEY(REPLYNUM),
   CONSTRAINT REPLY_EVENT_MEMBER_FK FOREIGN KEY(MEMBERID) REFERENCES TABLE_USER(MEMBERID),
   CONSTRAINT REPLY_EVENT_FK FOREIGN KEY(BOARDNUM) REFERENCES TABLE_BOARD_EVENT(BOARDNUM)
);
