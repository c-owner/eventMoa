...
CREATE TABLE TABLE_BOARD_freeTalk(
	...
	CONSTRAINT BOARD_freeTalk_FK FOREIGN KEY(BOARD_ID) REFERENCES TABLE_USER(MEMBER_ID)
);
...
CREATE TABLE TABLE_REPLY_freeTalk(
   ...
   CONSTRAINT REPLY_freeTalk_MEMBER_FK FOREIGN KEY(MEMBER_ID) REFERENCES TABLE_USER(MEMBER_ID)
);
...
create table TABLE_BOARD_POINT(
	...
	MEMBER_ID varchar2(100),
	constraint POINT_MEMBER_FK foreign KEY(MEMBER_ID) references TABLE_USER(MEMBER_ID)
);
...
CREATE TABLE TABLE_BOARD_EVENT(
	...
	CONSTRAINT BOARD_EVENT_FK FOREIGN KEY(BOARD_ID) REFERENCES TABLE_USER(MEMBER_ID)
);
...
CREATE TABLE TABLE_REPLY_EVENT (
   ...
   CONSTRAINT REPLY_EVENT_MEMBER_FK FOREIGN KEY(MEMBER_ID) REFERENCES TABLE_USER(MEMBER_ID),
